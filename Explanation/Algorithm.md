...
این تصویر تفاوت بین سه مدل را در عمل نشان می‌دهد:

1.رگرسیون معمولی (نمودار چپ): بیش‌ازحد به داده‌های آموزشی چسبیده، خطای تست بالا (۳.۷۶) → Overfitting شدید

2.الگوریتم Lasso (نمودار وسط): مدل ساده‌تر و انعطاف‌پذیر، بهترین عملکرد (خطای تست ۲.۲۷) → انتخاب ویژگی هوشمند

3.الگوریتم Ridge (نمودار راست): عملکرد متوسط (خطای تست ۲.۸۲) → تعادل محافظه‌کارانه

**نتیجه کلیدی: وقتی داده‌ها پیچیده‌اند، ساده‌سازی مدل (با Lasso) بهتر از پیچیده‌سازی بی‌رویه (رگرسیون معمولی) عمل می‌کند.**
